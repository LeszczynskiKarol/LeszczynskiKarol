# Hey there, I'm Karol 👋

<div align="center">
  
  [![Portfolio](https://img.shields.io/badge/Portfolio-karol--leszczynski.pl-0891b2?style=for-the-badge&logo=google-chrome&logoColor=white)](https://karol-leszczynski.pl)
  [![Email](https://img.shields.io/badge/Email-kontakt@karol--leszczynski.pl-EA4335?style=for-the-badge&logo=gmail&logoColor=white)](mailto:kontakt@karol-leszczynski.pl)
  
</div>

## 🚀 About Me

Full-stack dev from Poland. I ship SaaS products, Astro sites, and LaTeX textbooks.
```
// 15 lat temu pisałem teksty SEO za 5 zł/1000 znaków.
// Teraz stawiam SaaS-y na AWS i kompiluję podręczniki w LaTeX-u.
// Po drodze: WordPress → Node.js → React → Astro → "czemu to nie działa na produkcji?"
// Baza: Toruń. Stack: whatever ships.
```

## 💼 Featured Projects

### 🎓 **MaturaPolski.pl** 
AI platform for Polish language exam preparation – essays, analysis, personalized feedback
`React` `TypeScript` `Node.js` `Claude AI` `AWS`

### ✍️ **Smart-Copy.ai**
Advanced AI content generation platform with multi-agent architecture
`React 18` `TypeScript` `Vite` `Claude AI` `PostgreSQL`

### ⚡ **Silniki-Elektryczne.com.pl**
E-commerce platform with Allegro integration – **+40% sales increase** after migration from WooCommerce
`Next.js` `Node.js` `PostgreSQL` `Allegro API`

### ✏️ **Interpunkcja.com.pl**
AI punctuation checker with grammar explanations and rule learning
`React 18` `TypeScript` `Vite` `Claude AI`

### 🏠 **GrandKuchnie.pl**
Professional website for custom kitchen manufacturer with portfolio system
`Next.js` `Prisma` `TypeScript` `Tailwind CSS`

### 🌐 **TorWeb.pl**
Web agency site with full GDPR cookies system & Google Consent Mode v2
`Astro 5` `TypeScript` `Tailwind CSS`

### 📚 **Educational Publications**
11 professional workbooks in LaTeX/Quarto for vocational schools – ready for print
`LaTeX` `Quarto` `Python` `R Markdown`

## 🛠️ Tech Stack

### Frontend
![React](https://img.shields.io/badge/-React-61DAFB?style=flat-square&logo=react&logoColor=black)
![Next.js](https://img.shields.io/badge/-Next.js-000000?style=flat-square&logo=next.js&logoColor=white)
![TypeScript](https://img.shields.io/badge/-TypeScript-3178C6?style=flat-square&logo=typescript&logoColor=white)
![Tailwind CSS](https://img.shields.io/badge/-Tailwind_CSS-38B2AC?style=flat-square&logo=tailwind-css&logoColor=white)
![Astro](https://img.shields.io/badge/-Astro-FF5D01?style=flat-square&logo=astro&logoColor=white)

### Backend
![Node.js](https://img.shields.io/badge/-Node.js-339933?style=flat-square&logo=node.js&logoColor=white)
![Express](https://img.shields.io/badge/-Express-000000?style=flat-square&logo=express&logoColor=white)
![Python](https://img.shields.io/badge/-Python-3776AB?style=flat-square&logo=python&logoColor=white)
![PHP](https://img.shields.io/badge/-PHP-777BB4?style=flat-square&logo=php&logoColor=white)

### Database & Cloud
![MongoDB](https://img.shields.io/badge/-MongoDB-47A248?style=flat-square&logo=mongodb&logoColor=white)
![PostgreSQL](https://img.shields.io/badge/-PostgreSQL-336791?style=flat-square&logo=postgresql&logoColor=white)
![AWS](https://img.shields.io/badge/-AWS-232F3E?style=flat-square&logo=amazon-aws&logoColor=white)
![Vercel](https://img.shields.io/badge/-Vercel-000000?style=flat-square&logo=vercel&logoColor=white)

### AI & Automation
![Claude](https://img.shields.io/badge/-Claude_AI-D97757?style=flat-square&logo=anthropic&logoColor=white)
![OpenAI](https://img.shields.io/badge/-OpenAI-412991?style=flat-square&logo=openai&logoColor=white)
![Make](https://img.shields.io/badge/-Make.com-6D3EE6?style=flat-square&logo=make&logoColor=white)
![Airtable](https://img.shields.io/badge/-Airtable-18BFFF?style=flat-square&logo=airtable&logoColor=white)

### Publishing & Documentation
![LaTeX](https://img.shields.io/badge/-LaTeX-008080?style=flat-square&logo=latex&logoColor=white)
![Quarto](https://img.shields.io/badge/-Quarto-75AADB?style=flat-square&logo=quarto&logoColor=white)

## 🎯 What I'm Up To

- 🎓 Scaling **MaturaPolski.pl** for 2025 exam season
- 🤖 Building AI agents for content automation
- 📚 Publishing educational workbooks (11 ready for print)
- 🛒 Migrating e-commerce from WooCommerce to custom Next.js solutions
- ⚡ Integrating Claude AI & OpenAI into business workflows

## 📫 Let's Connect

Got an interesting project? Let's talk about:
- **🌐 Modern web applications** with Next.js, Astro & TypeScript
- **🤖 AI integrations** for your business
- **⚡ Process automation** to save time and money
- **📚 Professional publishing** with LaTeX/Quarto
- **🛒 E-commerce solutions** and WooCommerce migrations

<div align="center">
  
  **[📧 kontakt@karol-leszczynski.pl](mailto:kontakt@karol-leszczynski.pl)** | **[📱 +48 509 370 772](tel:+48509370772)**
  
  ---
  
  <sub>From copywriting to code – crafting digital solutions since 2009</sub>
  
</div>
